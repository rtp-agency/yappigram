...
function ChatsContent() {
  ...
  const applyTemplate = async (tpl: Template) => {
    ...
    // Block-based templates — fire ONE request, backend handles all blocks
    // with inter-block delays server-side as a background task. Each block
    // triggers a WS "new_message" event so the chat view updates in real
    // time even if the user navigates away. This replaced a client-side
    // loop with setTimeout that broke when the user left the chat mid-send.
    if (tpl.blocks_json && tpl.blocks_json.length > 0) {
      try {
        await api(`/api/messages/${selected.id}/send-template-blocks?template_id=${tpl.id}`, {
          method: "POST",
        });
        // Server returns immediately (background task). Messages arrive
        // via WS — no need to manually refetch.
      } catch (e: any) { alert(e.message); }
      return;
    }
